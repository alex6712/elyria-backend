...
class ChangeProfileUseCase:
    ...
    async def execute(self, input: ChangeProfileInput) -> None:
        ...
        if not is_set(input.display_name) and not is_set(input.avatar_url):
        ...
        async with self._uow:
            ...
            if is_set(input.display_name):
                profile.change_display_name(input.display_name)

            if is_set(input.avatar_url):
                profile.change_avatar_url(input.avatar_url)
