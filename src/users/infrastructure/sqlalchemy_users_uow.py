...
class SqlAlchemyUsersUnitOfWork:
    """SQLAlchemy-реализация Unit of Work контекста Users.

    Инкапсулирует управление соединением и транзакцией СУБД для одной
    единицы работы (Unit of Work) в ограниченном контексте Users.
    ...
    def _ensure_active_connection(self) -> AsyncConnection:
        ...
        if self._connection is None:
            raise UnitOfWorkNotEnteredError(
                "UsersUnitOfWork must be entered before use."
            )
