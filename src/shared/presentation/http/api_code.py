...
class APICode(StrEnum):
    ...
    INACTIVE_USER = auto()
    """При попытке выполнить операцию, требующую активной учётной
    записи, для неактивного пользователя (вход или обновление
    сессии деактивированного пользователя)."""
    ...
    COMPROMISED_PASSWORD = auto()
    """При попытке регистрации с паролем, который встречается
    ...
    """При попытке обновления пары токенов по сессии, срок жизни
    которой истёк: срок жизни сессии независим от срока жизни
    токена обновления."""
    ...
    """При попытке обновления пары токенов по сессии, которая была
    принудительно завершена (logout)."""
    ...
    """При конфликте оптимистичной блокировки: версия агрегата
    в хранилище изменилась с момента его загрузки."""
